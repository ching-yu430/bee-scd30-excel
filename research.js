...
calculate=function(){
 ...
 for(const row of d.rows){if(state.querySubset&&!state.querySubset[source]?.has(row))continue;const date=rowDate(row,d);if(!date){state.stats.invalidTime++;continue}if(+date<start||+date>end)continue;
...
function chartTimeBounds(datasets,start,end,mode){
 let lo=Infinity,hi=-Infinity;for(const d of datasets)for(const p of d.data){if(p&&p.y!==null&&Number.isFinite(p.x)){lo=Math.min(lo,p.x);hi=Math.max(hi,p.x)}}
 if(hi>lo)return {min:lo,max:hi};
 const step=({raw:60000,'5min':300000,'15min':900000,hour:3600000,day:86400000})[mode]||3600000;
 if(Number.isFinite(start)&&Number.isFinite(end)&&end>start)return {min:Number.isFinite(lo)?Math.min(lo,start):start,max:Number.isFinite(hi)?Math.max(hi,end):end};
 const center=Number.isFinite(lo)?lo:Number.isFinite(start)?start:null;
 return center===null?{}:{min:center-step/2,max:center+step/2};
}
createChart=function(canvas,config){
 config.options.layout={padding:{left:24,right:32,top:18,bottom:14}};
 ...
 else if(value('chartMode')!=='overlay'){Object.assign(config.options.scales.x,chartTimeBounds(config.data.datasets,+new Date(value('startDate')),+new Date(value('endDate')),effectiveMode()))}
...
function exportCharts(charts){
 ...
 out.toBlob(blob=>{if(!blob){$('analysisStatus').textContent='圖片產生失敗，請縮小匯出尺寸。';return}if(state.exportUrl)URL.revokeObjectURL(state.exportUrl);state.exportUrl=URL.createObjectURL(blob);let preview=$('exportPreview');if(!preview){preview=document.createElement('details');preview.id='exportPreview';$('chartCard').appendChild(preview)}preview.innerHTML='<summary>檢視最近匯出的圖片</summary><a download="hive-research-v16.png">儲存 PNG 圖片</a><img alt="匯出圖表預覽" style="width:100%;height:auto">';preview.querySelector('img').src=state.exportUrl;const a=preview.querySelector('a');a.href=state.exportUrl;a.click();$('analysisStatus').textContent='已產生 '+width+' × '+height+' px 圖片，可在圖表下方預覽或再次儲存。'},'image/png');
}

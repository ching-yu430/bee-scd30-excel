...
function exportCharts(charts){
 ...
 out.toBlob(blob=>{if(!blob){$('analysisStatus').textContent='圖片產生失敗，請縮小匯出尺寸。';return}if(state.exportUrl)URL.revokeObjectURL(state.exportUrl);state.exportUrl=URL.createObjectURL(blob);let preview=$('exportPreview');if(!preview){preview=document.createElement('details');preview.id='exportPreview';$('chartCard').appendChild(preview)}preview.innerHTML='<summary>檢視最近匯出的圖片</summary><a download="hive-research-v12.png">儲存 PNG 圖片</a><img alt="匯出圖表預覽" style="width:100%;height:auto">';preview.querySelector('img').src=state.exportUrl;const a=preview.querySelector('a');a.href=state.exportUrl;a.click();$('analysisStatus').textContent='已產生 '+width+' × '+height+' px 圖片，可在圖表下方預覽或再次儲存。'},'image/png');
}

...
const preciseLabels=['startDate','endDate'].map(id=>$(id).closest('label'));for(const id of ['startDate','endDate'])$(id).step='0.001';for(const el of preciseLabels)periodPanel.appendChild(el);
document.querySelector('#controlsCard .controls-grid').before(periodPanel);
settingIds.push('periodMode');
let periodSnapshot='';
function syncPeriodUI(){const days=value('periodMode')==='days';$('dayStartLabel').hidden=!days;$('dayEndLabel').hidden=!days;$('singleDay').hidden=!days;preciseLabels.forEach(el=>el.hidden=days);$('dayStart').value=value('startDate').slice(0,10);$('dayEnd').value=value('endDate').slice(0,10);periodSnapshot=value('startDate')+'|'+value('endDate')}

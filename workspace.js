...
const editor=document.createElement('section');editor.className='chart-editor';editor.innerHTML='<div class="editor-heading"><h3>編輯圖表</h3><span>改完即時預覽 · 不改變原始資料</span></div><div id="quickLabels" class="quick-labels"></div><p class="help-text">點選標示：點曲線附近的資料點可加入／取消數值。精確指定時間請展開各圖下方的「指定要顯示數值的時間點」。最高／最低是每條線在目前篩選與平均後的極值；同值時標示最早的一點。指定點仍會保留。</p>';
...
const modeDescriptions={continuous:'按日期先後看變化：溫度、濕度、CO₂ 各一張圖，可比較內外部。',daily:'每天分開看：每個日期、每個指標各一張圖。建議先選少量日期。',combined:'把三個指標放在同一張圖，各自使用不同單位的縱軸。',co2hour:'看一天的平均規律：把所選多天的 08 時資料合在一起平均，依此產生 00–23 時的 24 個平均值。',overlay:'比較不同日的同一時間：例如 7/22 和 7/23 各畫一條線，對齊 00–24 時。每條線代表一天，不會把多天平均成一條線；只使用內部資料。',difference:'看蜂箱比外面高或低多少：同時段內部平均 − 外部平均。只比較溫度與濕度，需要外站資料。'};
